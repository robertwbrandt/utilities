#!/usr/bin/python2.7
import datetime, bisect, re

leaseFile = '/var/lib/dhcp/db/dhcpd.leases'
configFile = '/etc/dhcpd.conf'
ldapConfigFile = '/var/log/dhcp-ldap-startup.log'
...
def ip_to_num(ip_address):
	x = map(int,ip_address.split("."))
	return x[0]*2**24+x[1]*2**16+x[2]*2**8+x[3]


##############################################################################


myfile = open(leaseFile, 'r')
...
# Calculate Total Leases Available
pool_size = 0
for pool in get_pools(config):
	for ip_range in get_range(pool):
		pool_size += (ip_to_num(ip_range[1]) - ip_to_num(ip_range[0])) + 1
